...
const FundSessionFilter = (props: any): any => {
    const [ProjectInfoLoaded, setProjectInfoLoaded] = React.useState(false);
    const [ProjectData, setProjectData] = React.useState(null);
    const {SessionFilter, ActiveProject} = props;
    const sessions = APP.FUNCTIONS.GET_SESSION_YEARS();

    const handleGetProjectInfo = () => {
        new APP.SERVICES.PROJECT().GetProject(ActiveProject.id).then(({data}) => {
            if(data.success) {
                ...
                <div className={__css_YearAreaBox}>
                    {
                        sessions.map((data, i) => (
                            <div key={i} className={__css_YearArea}>
                                ...
                                <Link onClick={() => {
                                    new APP.SERVICES.SESSION_FILTER().setSessionFilterDate({
                                        StartYear: data.startYear,
                                        EndYear: data.endYear,
                                    });
                                }} to={APP.ROUTES.PRIVATE[props.SessionFilter.FundName + '_BALANCE_SHEET_SESSION_REPORT']}>
...
const mapStateToProps = (state: any) => {
    return {
        SessionFilter: state.SESSION_FILTER,
        ActiveProject: state.SWITCH_PROJECT,
    }
};

export default connect(mapStateToProps, null)(withRouter(FundSessionFilter));
