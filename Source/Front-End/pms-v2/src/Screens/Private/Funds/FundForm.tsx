...
class FundForm extends React.Component<any, any> {
    public state: any;
    private readonly fund_id: number;

    public constructor(props: any) {
        super(props);

        this.fund_id = typeof props.match.params.id === 'undefined' ? 0 : Number(props.match.params.id);
        ...
        this.getProjectFundParentItems = this.getProjectFundParentItems.bind(this);
    ...
    protected getFundData(): void {
        new APP.SERVICES.FUND().GetFund(this.fund_id).then(({data}) => {
    ...
    protected getSections(): void {
        new APP.SERVICES.FUND().GetSections().then(({data}) => {
    ...
    protected getItemHeads(): void {
        new APP.SERVICES.FUND().GetItemHeads().then(({data}) => {
    ...
    protected getUnits(): void {
        new APP.SERVICES.FUND().GetUnits().then(({data}) => {
    ...
    protected getSuppliers(): void {
        new APP.SERVICES.FUND().GetSuppliers().then(({data}) => {
    ...
    protected getProjectFundParentItems(): void {
        new APP.SERVICES.FUND().GetProjectFundParentItems().then(({data}) => {
    ...
    protected handleSubmitFund(): void {
        new APP.SERVICES.FUND().SaveFund({
            fund_id: this.fund_id,
            fund_type: this.state.FundType,
    ...
    public componentDidMount(): void {
        ...
        this.getContractors();
        this.getSuppliers();
        this.getProjectFundParentItems();

        if (this.state.EditMode) {
            this.getFundData();
        }

        document.title = "Fund Form";
    }
            ...
            <React.Fragment>
                <Card className={__css_card}>
                    <div className={__css_cardTitle}>
                        <div className={"title"}>
                            {this.state.FundType} FUND {this.state.EditMode ? "EDIT" : "CREATE"}
                            ...
                            <Row>
                               ...
                               {/* <Col md={6}>
                                ...
                                <Col md={12} style={{textAlign: "right"}}>
                                    <button className={__css_SaveFundBtn} onClick={this.handleSubmitFund}>Submit
                                    </button>
                                </Col>
                            </Row>
                        </Card.Text>
