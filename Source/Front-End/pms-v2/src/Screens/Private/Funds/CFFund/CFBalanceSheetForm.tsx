...
class CFBalanceSheetForm extends React.Component<any, any> {
    public state: any;
    ...
    public constructor(props: any) {
        ...
        this.handleSubmitCFFundBalanceSheet = this.handleSubmitCFFundBalanceSheet.bind(this);
        this.getProjects = this.getProjects.bind(this);
    }

    protected handleChangeFundsObject(index: number, key: string, value: any) {
        const CFFundList = this.state.CFFundList;
        CFFundList[index][key] = value;
        this.setState({CFFundList: CFFundList});
    }
    ...
    protected getProjectFunds(projectId: string | number | null = 0): void {
        new APP.SERVICES.BALANCE_SHEET().GetTasks('CF', projectId).then(({data}) => {

            const responseData = data.data;
            if (typeof responseData !== 'undefined' && responseData !== null) {
                this.setState({
                    CFFundList: this.generateFundsArrayOfObjects(responseData),
                });
            }
    ...
    protected handleSubmitCFFundBalanceSheet(): void {
        new APP.SERVICES.BALANCE_SHEET().SaveBalanceSheet({
            Session: this.state.Session,
            FundList: this.state.CFFundList,
            FundType: '',
        }).then(({data}) => {
            ...
            <React.Fragment>
                <Card className={__css_card}>
                    <div className={__css_cardTitle}>
                        <div className={"title"}>
                            CF BALANCE SHEET {this.state.EditMode ? "EDIT" : "CREATE"}
                        ...
                        <Card.Text className={__css_cardText}>
                            <Row>
                                ...
                                <Col md={12} style={{textAlign: "right"}}>
                                    <button className={__css_SaveCFFundBtn} onClick={this.handleSubmitCFFundBalanceSheet}>Submit
                                    </button>
                                </Col>
                            </Row>
                        </Card.Text>
...
export default withRouter(CFBalanceSheetForm);
