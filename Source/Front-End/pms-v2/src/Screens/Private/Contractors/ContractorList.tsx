...
class Contractors extends React.Component<any, any> {
    public state: any;
    static propTypes: { ReceiverMode: Requireable<boolean>; ReceiverData: Requireable<any[]> };
    ...
    public constructor(props: any) {
        ...
        this.state = {
            ...
            Contractors: props.ReceiverMode ? props.ReceiverData : [],

            SelectedContractorIdForDeleteAction: 0,
        };

        this.handleGetContractors = this.handleGetContractors.bind(this);
    ...
    protected handleGetContractors() {
        new APP.SERVICES.CONTRACTOR().GetContractors({
            ...
            if (typeof data.data !== 'undefined') {
                ...
                if (typeof responseData.ListData !== 'undefined') {
                    this.setState({
                        Contractors: responseData.ListData,
                    });
                }
            }
    ...
    protected handleDeleteContractor() {
        new APP.SERVICES.CONTRACTOR().DeleteContractor(this.state.SelectedContractorIdForDeleteAction).then(({data}) => {
            if (data.success) {
                this.handleGetContractors();
            }
    ...
    protected handleShowDeleteWarning(id: number = 0) {
        const self = this;
        this.setState({
            SelectedContractorIdForDeleteAction: id,
        }, () => {
            new APP.SERVICES.CORE.ALERT_DIALOG().Show({
                show: true,
                style: {fontSize: 12},
    ...
    public componentDidMount(): void {
        if(!this.props.ReceiverMode) {
            this.handleGetContractors();
        }

        //Scroll to the top of the document
        window.scrollTo(0, 0);
        document.title = "Contractor List";
    }
                ...
                <div style={{marginTop: 15}}>
                    <Row>
                        ...
                        <Col md={4}>
                            <div style={{textAlign: "right"}}>
                                <Link to={APP.ROUTES.PRIVATE.CREATE_CONTRACTOR}>
                                    <button className={__css_CreateContractorBtn}>Create Contractor</button>
                                </Link>
                            </div>
                        </Col>
                    </Row>
                    <div style={{background: '#ffffff', padding: 5}}>
                        <Table striped bordered hover className={__css_Table} responsive={true} size={"sm"}>
                            ...
                            <tbody>
                            {this.state.Contractors.map((Contractor: any, index: number) => (
                                <tr key={index}>
                                    <td>{index + 1}</td>
                                    <td>{Contractor.user.name}</td>
                                    <td>{Contractor.owner_name}</td>
                                    <td>{Contractor.it}</td>
                                    <td className={"center"}>{Contractor.owner_nid}</td>
                                    <td className={"center"}>
                                        {
                                            Contractor.license ?
                                                <a href={APP.CONFIG.CDN_ROOT + '/' + Contractor.license}
                                                   target={"__blank"}>
                                    ...
                                    <td>
                                        <Link to={APP.ROUTES.PRIVATE.CONTRACTOR_DETAILS + '/' + Contractor.id} title={"View"}><FontAwesome
                                            name={"eye"} className={"grid-action-button grid-action-edit"}/></Link>
                                        <Link to={APP.ROUTES.PRIVATE.EDIT_CONTRACTOR + '/' + Contractor.id}><FontAwesome
                                            name={"edit"} className={"grid-action-button grid-action-edit"}/></Link>
                                        <FontAwesome name={"trash"}
                                                     className={"grid-action-button grid-action-delete"}
                                                     onClick={() => this.handleShowDeleteWarning(Contractor.id)}/>
                                    </td>
                                </tr>
                            ))}
                            </tbody>
...
Contractors.propTypes = {
    ReceiverMode: PropTypes.bool,
    ReceiverData: PropTypes.array,
};
