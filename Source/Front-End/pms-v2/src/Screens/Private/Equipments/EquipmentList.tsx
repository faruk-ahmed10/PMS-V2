...
class EquipmentList extends React.Component<any, any> {
    public state: any;
    ...
    public constructor(props: any) {
        ...
        this.state = {
            ...
            SelectedEquipmentIdForDeleteAction: 0,
        };

        this.handleGetEquipment = this.handleGetEquipment.bind(this);
    ...
    protected handleGetEquipment() {
        new APP.SERVICES.EQUIPMENTS().GetEquipments({
            ...
            if (typeof data.data !== 'undefined') {
                ...
                if (typeof responseData.ListData !== 'undefined') {
                    this.setState({
                        Equipments: responseData.ListData,
                    });
                }
            }
    ...
    protected handleDeleteEquipment() {
        new APP.SERVICES.EQUIPMENTS().DeleteEquipment(this.state.SelectedEquipmentIdForDeleteAction).then(({data}) => {
            if (data.success) {
                this.handleGetEquipment();
            }
    ...
    protected handleShowDeleteWarning(id: number = 0) {
        const self = this;
        this.setState({
            SelectedEquipmentIdForDeleteAction: id,
        }, () => {
            new APP.SERVICES.CORE.ALERT_DIALOG().Show({
                show: true,
                style: {fontSize: 12},
                ...
                <div style={{marginTop: 15}}>
                    <Row>
                        ...
                        <Col md={4}>
                            <div style={{textAlign: "right"}}>
                                <Link to={APP.ROUTES.PRIVATE.CREATE_EQUIPMENTS}>
                                    <button className={__css_CreateContractorBtn}>Create Equipment</button>
                                </Link>
                            </div>
                        </Col>
                    </Row>
                    <div style={{background: '#ffffff', padding: 5}}>
                        <Table striped bordered hover className={__css_Table} responsive={true} size={"sm"}>
                            ...
                            <tbody>
                            {this.state.Equipments.map((Equipment: any, index: number) => {
                                const project_name = (typeof Equipment.project !== 'undefined' && Equipment.project !== null && typeof Equipment.project.name !== 'undefined' && Equipment.project.name !== null) ? Equipment.project.name : '';
                                const unit_name = (typeof Equipment.unit !== 'undefined' && Equipment.unit !== null && typeof Equipment.unit.name !== 'undefined' && Equipment.unit.name !== null) ? Equipment.unit.name : '';
                                const category_name = (typeof Equipment.category !== 'undefined' && Equipment.category !== null && typeof Equipment.category.name !== 'undefined' && Equipment.category.name !== null) ? Equipment.category.name : '';
                                return (
                                    <tr key={index}>
                                        <td>{index + 1}</td>
                                        <td>{project_name}</td>
                                        ...
                                        <td>{Equipment.qty}</td>
                                        <td>{Equipment.ba_no}</td>
                                        <td>{unit_name}</td>
                                        <td>{category_name}</td>
                                        <td>{Equipment.purchase_date}</td>
                                        <td>{Equipment.receive_date}</td>
                                        <td>{Equipment.location}</td>
                                        <td className={"center"}>
                                            <a href={APP.CONFIG.CDN_ROOT + '/' + Equipment.image} target={"__blank"}>
                                                <FontAwesome name={"file"} style={{color: "blue", cursor: "pointer"}} />
                                            </a>
                                        </td>
                                        <td>
                                            <Link to={APP.ROUTES.PRIVATE.EDIT_EQUIPMENTS + '/' + Equipment.id}><FontAwesome
                                                name={"edit"} className={"grid-action-button grid-action-edit"}/></Link>
                                            <FontAwesome name={"trash"}
                                                         className={"grid-action-button grid-action-delete"}
                                                         onClick={() => this.handleShowDeleteWarning(Equipment.id)}/>
                                        </td>
                                    </tr>
                                )
                            })}
