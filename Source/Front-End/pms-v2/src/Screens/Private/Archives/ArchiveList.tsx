...
class EquipmentList extends React.Component<any, any> {
    ...
    public constructor(props: any) {
        ...
        this.state = {
            ...
            SelectedArchivesIdForDeleteAction: 0,
        };
    ...
    protected handleGetEquipment() {
        new APP.SERVICES.ARCHIVES().GetArchives({
            ...
            if (typeof data.data !== 'undefined') {
                ...
                if (typeof responseData.ListData !== 'undefined') {
                    this.setState({
                        Archives: responseData.ListData,
                    });
                }
            }
    ...
    protected handleDeleteEquipment() {
        new APP.SERVICES.ARCHIVES().DeleteArchive(this.state.SelectedEquipmentIdForDeleteAction).then(({data}) => {
            if (data.success) {
                this.handleGetEquipment();
            }
                ...
                <div style={{marginTop: 15}}>
                    <Row>
                        ...
                        <Col md={4}>
                            <div style={{textAlign: "right"}}>
                                <Link to={APP.ROUTES.PRIVATE.CREATE_ARCHIVES}>
                                    <button className={__css_CreateContractorBtn}>Create Archive</button>
                                </Link>
                            </div>
                        </Col>
                    </Row>
                    <div style={{background: '#ffffff', padding: 5}}>
                        <Table striped bordered hover className={__css_Table} responsive={true} size={"sm"}>
                            ...
                            <tbody>
                            {this.state.Archives.map((Archive: any, index: number) => {
                                const project_name = (typeof Archive.project !== 'undefined' && Archive.project !== null && typeof Archive.project.name !== 'undefined' && Archive.project.name !== null) ? Archive.project.name : '';

                                const head_name = (typeof Archive.head !== 'undefined' && Archive.head !== null && typeof Archive.head.name !== 'undefined' && Archive.head.name !== null) ? Archive.head.name : '';
                                return (
                                    <tr key={index}>
                                        <td>{index + 1}</td>
                                        <td>{project_name}</td>
                                        ...
                                        <td>{Archive.title}</td>
                                        <td className={"center"}>
                                            <a href={APP.CONFIG.CDN_ROOT + '/' + Archive.document} target={"__blank"}>
                                                <FontAwesome name={"file"} style={{color: "blue", cursor: "pointer"}} />
                                            </a>
                                        </td>
                                        <td>
                                            <Link to={APP.ROUTES.PRIVATE.EDIT_ARCHIVES + '/' + Archive.id}><FontAwesome
                                                name={"edit"} className={"grid-action-button grid-action-edit"}/></Link>
                                            <FontAwesome name={"trash"}
                                                         className={"grid-action-button grid-action-delete"}
                                                         onClick={() => this.handleShowDeleteWarning(Archive.id)}/>
                                        </td>
                                    </tr>
                                )
                            })}
