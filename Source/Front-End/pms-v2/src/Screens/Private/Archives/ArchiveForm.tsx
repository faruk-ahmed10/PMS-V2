...
class ArchiveForm extends React.Component<any, any> {
    public state: any;
    private readonly archive_id: number;

    public constructor(props: any) {
        super(props);

        this.archive_id = typeof props.match.params.id === 'undefined' ? 0 : Number(props.match.params.id);
        ...
        this.state = {
            ...
            EditMode: this.archive_id > 0,
        };

        this.getArchiveData = this.getArchiveData.bind(this);
        this.handleSubmitArchive = this.handleSubmitArchive.bind(this);
    }

    protected getArchiveData(): void {
        new APP.SERVICES.ARCHIVES().GetArchive(this.archive_id).then(({data}) => {

            const responseData = data.data;
            if (typeof responseData !== 'undefined' && responseData !== null) {
                this.setState({
                    Title: responseData.title,
                    HeadID:responseData.head_id,
    ...
    protected getProjectList(): void {
        new APP.SERVICES.ARCHIVES().GetProject().then(({data}) => {
    ...
    protected getHeadList(): void {
        new APP.SERVICES.ARCHIVES().GetHeads().then(({data}) => {

            const responseData = data.data;
            if (typeof responseData !== 'undefined') {
                this.setState({
                    HeadList: APP.FUNCTIONS.DROPDOWN_OBJECT_MAKER(responseData, 'name', 'id'),
                });
            }
    ...
    protected handleSubmitArchive(): void {
        new APP.SERVICES.ARCHIVES().SaveArchive({
                    ...
                    <Card.Body className={__css_cardBody}>
                        <Card.Text className={__css_cardText}>
                            <Row>
                                ...
                                <Col md={6}>
                                    <Select options={this.state.HeadList}
                                            defaultValue={""}
                                            label={"Select Head"}
                                            name={"unit_id"}
                                ...
                                <Col md={12} style={{textAlign: "right"}}>
                                    <button className={__css_SaveContractorBtn}
                                            onClick={this.handleSubmitArchive}>Submit
                                    </button>
                                </Col>
                            </Row>
                        </Card.Text>
...
export default withRouter(ArchiveForm);