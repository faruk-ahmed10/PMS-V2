...
class DstnChartList extends React.Component<any, any> {
    public state: any;
    ...
    public constructor(props: any) {
        ...
        this.state = {
            ...
            SelectedDstnIdForDeleteAction: 0,
        };

        this.handleGetDstn = this.handleGetDstn.bind(this);
        this.handleDeleteDstn = this.handleDeleteDstn.bind(this);
    ...
    protected handleGetDstn() {
        new APP.SERVICES.DSTN().GetDstns({
            ...
            if (typeof data.data !== 'undefined') {
                ...
                if (typeof responseData.ListData !== 'undefined') {
                    this.setState({
                        DstnCharts: responseData.ListData,
                    });
                }
            }
    ...
    protected handleDeleteDstn() {
        new APP.SERVICES.DSTN().DeleteDstn(this.state.SelectedDstnIdForDeleteAction).then(({data}) => {
            if (data.success) {
                this.handleGetDstn();
            }
    ...
    protected handleShowDeleteWarning(id: number = 0) {
        const self = this;
        this.setState({
            SelectedDstnIdForDeleteAction: id,
        }, () => {
            new APP.SERVICES.CORE.ALERT_DIALOG().Show({
                show: true,
                style: {fontSize: 12},
                ...
                <div style={{marginTop: 15}}>
                    <Row>
                        ...
                        <Col md={4}>
                            <div style={{textAlign: "right"}}>
                                <Link to={APP.ROUTES.PRIVATE.CREATE_DSTN}>
                                    <button className={__css_CreateContractorBtn}>Create DSTN Chart</button>
                                </Link>
                            </div>
                        </Col>
                    </Row>
                    <div style={{background: '#ffffff', padding: 5}}>
                        <Table striped bordered hover className={__css_Table} responsive={true} size={"sm"}>
                            ...
                            <tbody>
                            {this.state.DstnCharts.map((Dstn: any, index: number) => (
                                <tr key={index}>
                                    <td>{index + 1}</td>
                                    <td>{Dstn.project.name}</td>
                                    <td className={"center"}>
                                        <a href={APP.CONFIG.CDN_ROOT + '/' + Dstn.image} target={"__blank"}>
                                            <FontAwesome name={"file"} style={{color: "blue", cursor: "pointer"}} />
                                        </a>
                                    </td>
                                    <td>
                                        <Link to={APP.ROUTES.PRIVATE.EDIT_DSTN + '/' + Dstn.id}><FontAwesome
                                            name={"edit"} className={"grid-action-button grid-action-edit"}/></Link>
                                        <FontAwesome name={"trash"}
                                                     className={"grid-action-button grid-action-delete"}
                                                     onClick={() => this.handleShowDeleteWarning(Dstn.id)}/>
                                    </td>
                                </tr>
                            ))}
                            </tbody>
...
export default withRouter(DstnChartList);