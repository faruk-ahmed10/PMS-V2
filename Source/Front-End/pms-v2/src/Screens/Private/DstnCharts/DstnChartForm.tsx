...
class DstnChartForm extends React.Component<any, any> {
    public state: any;
    private readonly dstn_id: number;

    public constructor(props: any) {
        super(props);

        this.dstn_id = typeof props.match.params.id === 'undefined' ? 0 : Number(props.match.params.id);
        ...
        this.state = {
            ...
            EditMode: this.dstn_id > 0,
        };

        this.getDstnData = this.getDstnData.bind(this);
        this.handleSubmitDstn = this.handleSubmitDstn.bind(this);
    }

    protected getDstnData(): void {
        new APP.SERVICES.DSTN().GetDstn(this.dstn_id).then(({data}) => {
    ...
    protected getProjectList(): void {
        new APP.SERVICES.DSTN().GetProject().then(({data}) => {
    ...
    protected handleSubmitDstn(): void {
        new APP.SERVICES.DSTN().SaveDstn({

            Dstn_id: this.dstn_id,
            ...
            <React.Fragment>
                <Card className={__css_card}>
                    <div className={__css_cardTitle}>
                        <div className={"title"}>
                            DSTN {this.state.EditMode ? "EDIT" : "CREATE"}
                            ...
                            <Row>
                                ...
                                <Col md={12} style={{textAlign: "right"}}>
                                    <button className={__css_SaveContractorBtn}
                                            onClick={this.handleSubmitDstn}>Submit
                                    </button>
                                </Col>
                            </Row>
                        </Card.Text>
...
export default withRouter(DstnChartForm);