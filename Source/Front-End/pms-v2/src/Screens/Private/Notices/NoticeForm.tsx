...
class NoticeForm extends React.Component<any, any> {
    public state: any;
    private readonly notice_id: number;

    public constructor(props: any) {
        super(props);

        this.notice_id = typeof props.match.params.id === 'undefined' ? 0 : Number(props.match.params.id);
    ...
    protected getNoticeData(): void {
        new APP.SERVICES.NOTICES().GetNotice(this.notice_id).then(({data}) => {
            ...
            if (typeof responseData !== 'undefined' && responseData !== null) {
                this.setState({
                    ...
                    PublishDate: responseData.publish_date,
    ...
    protected handleSubmitNotice(): void {
        new APP.SERVICES.NOTICES().SaveNotice({

            notice_id: this.notice_id,
            title: this.state.Title,
            description: this.state.Description,
            publish_date: APP.FUNCTIONS.CONVERT_DATE(this.state.PublishDate, 'yyyy-mm-dd'),
            ...
            <React.Fragment>
                <Card className={__css_card}>
                    <div className={__css_cardTitle}>
                        <div className={"title"}>
                            NOTICE {this.state.EditMode ? "EDIT" : "CREATE"}
                    ...
                    <Card.Body className={__css_cardBody}>
                        <Card.Text className={__css_cardText}>
                            <Row>
                                ...
                                <Col md={6}>
                                    <DatePickerTextField label={"Publish Date"}
                                                         name={"publish_date"}
                                ...
                                <Col md={12} style={{textAlign: "right"}}>
                                    <button className={__css_SaveContractorBtn}
                                            onClick={this.handleSubmitNotice}>Submit
                                    </button>
                                </Col>
                            </Row>
                        </Card.Text>
