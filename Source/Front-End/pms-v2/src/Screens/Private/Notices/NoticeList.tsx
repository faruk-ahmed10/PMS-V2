...
class Notices extends React.Component<any, any> {
    public state: any;
    ...
    public constructor(props: any) {
        ...
        this.state = {
            ...
            SelectedNoticeIdForDeleteAction: 0,
        };

        this.handleGetNotice = this.handleGetNotice.bind(this);
    ...
    protected handleGetNotice() {
        new APP.SERVICES.NOTICES().GetNotices({
    ...
    protected handleDeleteNotice() {
        new APP.SERVICES.NOTICES().DeleteNotice(this.state.SelectedNoticeIdForDeleteAction).then(({data}) => {
            if (data.success) {
                this.handleGetNotice();
            }
    ...
    protected handleShowDeleteWarning(id: number = 0) {
        const self = this;
        this.setState({
            SelectedNoticeIdForDeleteAction: id,
        }, () => {
            new APP.SERVICES.CORE.ALERT_DIALOG().Show({
                show: true,
                style: {fontSize: 12},
                ...
                <div style={{marginTop: 15}}>
                    <Row>
                        ...
                        <Col md={4}>
                            <div style={{textAlign: "right"}}>
                                <Link to={APP.ROUTES.PRIVATE.CREATE_NOTICE}>
                                    <button className={__css_CreateContractorBtn}>Create Notice</button>
                                </Link>
                            </div>
                        </Col>
                    </Row>
                    <div style={{background: '#ffffff', padding: 5}}>
                        <Table striped bordered hover className={__css_Table} responsive={true} size={"sm"}>
                            ...
                            <tbody>
                            {this.state.Notices.map((Notice: any, index: number) => (
                                <tr key={index}>
                                    <td>{index + 1}</td>
                                    <Link to={APP.ROUTES.PRIVATE.VIEW_NOTICE + '/' + Notice.id}><td>{Notice.title}</td></Link>
                                    <td>{Notice.description}</td>
                                    <td>{APP.FUNCTIONS.CONVERT_DATE(Notice.publish_date,"dd-mm-yyyy")}</td>
                                    <td className={"center"}>
                                        <a href={APP.CONFIG.CDN_ROOT + '/' + Notice.file} target={"__blank"}>
                                            <FontAwesome name={"file"} style={{color: "blue", cursor: "pointer"}} />
                                        </a>
                                    </td>
                                    <td>{Notice.status}</td>
                                    <td>
                                        <Link to={APP.ROUTES.PRIVATE.VIEW_NOTICE + '/' + Notice.id}><FontAwesome
                                            name={"eye"} className={"grid-action-button grid-action-edit"}/></Link>
                                        <Link to={APP.ROUTES.PRIVATE.EDIT_NOTICE + '/' + Notice.id}><FontAwesome
                                            name={"edit"} className={"grid-action-button grid-action-edit"}/></Link>
                                        <FontAwesome name={"trash"}
                                                     className={"grid-action-button grid-action-delete"}
                                                     onClick={() => this.handleShowDeleteWarning(Notice.id)}/>
                                    </td>
                                </tr>
                            ))}
                            </tbody>
