...
class ItemHeadList extends React.Component<any, any> {
    public state: any;
    ...
    public constructor(props: any) {
        ...
        this.state = {
            ...
            SelectedItemHeadIdForDeleteAction: 0,

        };

        this.handleGetItemHead = this.handleGetItemHead.bind(this);
        this.handleDeleteItemHead = this.handleDeleteItemHead.bind(this);
    ...
    protected handleGetItemHead() {
        new APP.SERVICES.ITEM_HEAD().GetItemHeads({
            ...
            if (typeof data.data !== 'undefined') {
                ...
                if (typeof responseData.ListData !== 'undefined') {
                    this.setState({
                        ItemHeads: responseData.ListData,
                    });
    ...
    protected handleDeleteItemHead() {
        new APP.SERVICES.ITEM_HEAD().DeleteItemHead(this.state.SelectedItemHeadIdForDeleteAction).then(({data}) => {
            if (data.success) {
                this.handleGetItemHead();
            }
    ...
    protected handleShowDeleteWarning(id: number = 0) {
        const self = this;
        this.setState({
            SelectedItemHeadIdForDeleteAction: id,
        }, () => {
            new APP.SERVICES.CORE.ALERT_DIALOG().Show({
                show: true,
                style: {fontSize: 12},
            ...
            <React.Fragment>
                <div style={{marginTop: 15}}>
                    <Row>
                        <Col md={8}>
                            <h4>
                                <Link to={APP.ROUTES.PRIVATE.ITEM_HEADS}>Item Head List</Link>
                            </h4>
                        </Col>
                        <Col md={4}>
                            <div style={{textAlign: "right"}}>
                                <Link to={APP.ROUTES.PRIVATE.CREATE_ITEM_HEAD}>
                                    <button className={__css_CreateContractorBtn}>Create Item Head</button>
                                </Link>
                            </div>
                        </Col>
                    </Row>
                    <div style={{background: '#ffffff', padding: 5}}>
                        <Table striped bordered hover className={__css_Table} responsive={true} size={"sm"}>
                            ...
                            <tbody>
                            {this.state.ItemHeads.map((ItemHead: any, index: number) => (
                                <tr key={index}>
                                    <td>{index + 1}</td>
                                    <td>{ItemHead.name}</td>
                                    <td>
                                        <Link to={APP.ROUTES.PRIVATE.EDIT_ITEM_HEAD + '/' + ItemHead.id}>
                                            <FontAwesome
                                                name={"edit"} className={"grid-action-button grid-action-edit"}/>
                                        </Link>
                                        <FontAwesome name={"trash"}
                                                     className={"grid-action-button grid-action-delete"}
                                                     onClick={() => this.handleShowDeleteWarning(ItemHead.id)}/>
                                    </td>
                                </tr>
                            ))}
                            </tbody>
...
export default withRouter(ItemHeadList);