...
class ItemHeadForm extends React.Component<any, any> {
    public state: any;
    private readonly item_head_id: number;

    public constructor(props: any) {
        super(props);

        this.item_head_id = typeof props.match.params.id === 'undefined' ? 0 : Number(props.match.params.id);

        this.state = {
            NotFoundException: false,
            Name: '',

            EditMode: this.item_head_id > 0,
        };

        this.getItemHeadData = this.getItemHeadData.bind(this);
        this.handleSubmitItemHead = this.handleSubmitItemHead.bind(this);
    }

    protected getItemHeadData(): void {
        new APP.SERVICES.ITEM_HEAD().GetItemHead(this.item_head_id).then(({data}) => {
    ...
    protected handleSubmitItemHead(): void {
        new APP.SERVICES.ITEM_HEAD().SaveItemHead({
            item_head_id: this.item_head_id,
            ...
            <React.Fragment>
                <Card className={__css_card}>
                    <div className={__css_cardTitle}>
                        <div className={"title"}>
                            ITEM HEAD {this.state.EditMode ? "EDIT" : "CREATE"}
                            ...
                            <Row>
                                ...
                                <Col md={6} style={{textAlign: "left"}}>
                                    <button className={__css_SaveContractorBtn}
                                            onClick={this.handleSubmitItemHead}>Submit
                                    </button>
                                </Col>
                            </Row>
                        </Card.Text>
...
export default withRouter(ItemHeadForm);