...
class GalleryList extends React.Component<any, any> {
    public state: any;
    ...
    public constructor(props: any) {
        ...
        this.state = {
            ...
            SelectedGalleryIdForDeleteAction: 0,
        };
        ...
        new APP.SERVICES.GALLERYS().GetGallerys({
            ...
            if (typeof data.data !== 'undefined') {
                ...
                if (typeof responseData.ListData !== 'undefined') {
                    this.setState({
                        Gallerys: responseData.ListData,
                    });
                }
            }
    ...
    protected handleDeleteGallery() {
        new APP.SERVICES.GALLERYS().DeleteGallery(this.state.SelectedGalleryIdForDeleteAction).then(({data}) => {
            if (data.success) {
                this.handleGetGallery();
            }
    ...
    protected handleShowDeleteWarning(id: number = 0) {
        const self = this;
        this.setState({
            SelectedGalleryIdForDeleteAction: id,
        }, () => {
            new APP.SERVICES.CORE.ALERT_DIALOG().Show({
                show: true,
                style: {fontSize: 12},
                ...
                <div style={{marginTop: 15}}>
                        ...
                        <Col md={4}>
                            <div style={{textAlign: "right"}}>
                                <Link to={APP.ROUTES.PRIVATE.GALLERY_GRID}>
                                    <button className={__css_CreateContractorBtn}>Grid View</button>
                                </Link>
                                &nbsp;
                                <Link to={APP.ROUTES.PRIVATE.CREATE_GALLERY}>
                                    <button className={__css_CreateContractorBtn}>Create Gallery</button>
                                </Link>
                            </div>
                        </Col>
                    </Row>
                    <div style={{background: '#ffffff', padding: 5}}>
                        <Table striped bordered hover className={__css_Table} responsive={true} size={"sm"}>
                            ...
                            <tbody>
                            {this.state.Gallerys.map((Gallery: any, index: number) => {
                                const project_name = (typeof Gallery.project !== 'undefined' && Gallery.project !== null && typeof Gallery.project.name !== 'undefined' && Gallery.project.name !== null) ? Gallery.project.name : '';
                                return (
                                    <tr key={index}>
                                        <td>{index + 1}</td>
                                        <td>{project_name}</td>
                                        <td>{Gallery.title}</td>
                                        <td>
                                            <input type={"checkbox"} disabled={true} checked={Number(Gallery.is_featured) === 1} />
                                        </td>
                                        <td className={"center"}>
                                            <a href={APP.CONFIG.CDN_ROOT + '/' + Gallery.image} target={"__blank"}>
                                                <FontAwesome name={"file"} style={{color: "blue", cursor: "pointer"}} />
                                            </a>
                                        </td>
                                        <td>
                                            <Link to={APP.ROUTES.PRIVATE.EDIT_GALLERY + '/' + Gallery.id}><FontAwesome
                                                name={"edit"} className={"grid-action-button grid-action-edit"}/></Link>
                                            <FontAwesome name={"trash"}
                                                         className={"grid-action-button grid-action-delete"}
                                                         onClick={() => this.handleShowDeleteWarning(Gallery.id)}/>
                                        </td>
                                    </tr>
                                )
                            })}
