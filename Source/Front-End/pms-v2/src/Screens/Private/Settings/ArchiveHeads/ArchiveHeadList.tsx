...
class SectionList extends React.Component<any, any> {
    public state: any;
    ...
    public constructor(props: any) {
        ...
        this.state = {
            ...
            SelectedSectionIdForDeleteAction: 0,

        };

        this.handleGetSection = this.handleGetSection.bind(this);
    ...
    protected handleGetSection() {
        new APP.SERVICES.ARCHIVE_HEADS().GetArchiveHeads({
            ...
            if (typeof data.data !== 'undefined') {
                ...
                if (typeof responseData.ListData !== 'undefined') {
                    this.setState({
                        Sections: responseData.ListData,
                    });
    ...
    protected handleDeleteSection() {
        new APP.SERVICES.SECTIONS().DeleteSection(this.state.SelectedSectionIdForDeleteAction).then(({data}) => {
            if (data.success) {
                this.handleGetSection();
            }
    ...
    protected handleShowDeleteWarning(id: number = 0) {
        const self = this;
        this.setState({
            SelectedSectionIdForDeleteAction: id,
        }, () => {
            new APP.SERVICES.CORE.ALERT_DIALOG().Show({
                show: true,
                style: {fontSize: 12},
            ...
            <React.Fragment>
                <div style={{marginTop: 15}}>
                    <Row>
                        <Col md={8}>
                            <h4>
                                <Link to={APP.ROUTES.PRIVATE.SECTIONS}>Archive Head List</Link>
                            </h4>
                        </Col>
                        <Col md={4}>
                            <div style={{textAlign: "right"}}>
                                <Link to={APP.ROUTES.PRIVATE.CREATE_ARCHIVE_HEAD}>
                                    <button className={__css_CreateContractorBtn}>Create Archive Head</button>
                                </Link>
                            </div>
                        </Col>
                    </Row>
                    <div style={{background: '#ffffff', padding: 5}}>
                        <Table striped bordered hover className={__css_Table} responsive={true} size={"sm"}>
                            ...
                            <tbody>
                            {this.state.Sections.map((Head: any, index: number) => (
                                <tr key={index}>
                                    <td>{index + 1}</td>
                                    <td>{Head.name}</td>
                                    <td>
                                        <Link to={APP.ROUTES.PRIVATE.EDIT_ARCHIVE_HEAD + '/' + Head.id}>
                                            <FontAwesome
                                                name={"edit"} className={"grid-action-button grid-action-edit"}/>
                                        </Link>
                                        <FontAwesome name={"trash"}
                                                     className={"grid-action-button grid-action-delete"}
                                                     onClick={() => this.handleShowDeleteWarning(Head.id)}/>
                                    </td>
                                </tr>
                            ))}
                            </tbody>
