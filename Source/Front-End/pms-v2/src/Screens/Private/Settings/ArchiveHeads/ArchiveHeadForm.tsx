...
class ArchiveHeadForm extends React.Component<any, any> {
    public state: any;
    private readonly head_id: number;

    public constructor(props: any) {
        super(props);

        this.head_id = typeof props.match.params.id === 'undefined' ? 0 : Number(props.match.params.id);

        this.state = {
            NotFoundException: false,
            Name: '',

            EditMode: this.head_id > 0,
        };

        this.getArchiveHeadData = this.getArchiveHeadData.bind(this);
        this.handleSubmitArchiveHead = this.handleSubmitArchiveHead.bind(this);
    }

    protected getArchiveHeadData(): void {
        new APP.SERVICES.ARCHIVE_HEADS().GetArchiveHead(this.head_id).then(({data}) => {
    ...
    protected handleSubmitArchiveHead(): void {
        new APP.SERVICES.ARCHIVE_HEADS().SaveHead({
            ...
            <React.Fragment>
                <Card className={__css_card}>
                    <div className={__css_cardTitle}>
                        <div className={"title"}>
                            ARCHIVE HEAD {this.state.EditMode ? "EDIT" : "CREATE"}
                            ...
                            <Row>
                                ...
                                <Col md={6} style={{textAlign: "left"}}>
                                    <button className={__css_SaveContractorBtn}
                                            onClick={this.handleSubmitArchiveHead}>Submit
                                    </button>
                                </Col>
                            </Row>
                        </Card.Text>
...
export default withRouter(ArchiveHeadForm);