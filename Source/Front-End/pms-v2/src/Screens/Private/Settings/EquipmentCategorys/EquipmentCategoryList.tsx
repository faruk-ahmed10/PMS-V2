...
class EquipmentCategoryList extends React.Component<any, any> {
    public state: any;
    ...
    public constructor(props: any) {
        ...
        this.state = {
            ...
            SelectedEquCateIdForDeleteAction: 0,

        };

        this.handleGetEquCate = this.handleGetEquCate.bind(this);
        this.handleDeleteEquCate = this.handleDeleteEquCate.bind(this);
    ...
    protected handleGetEquCate() {
        new APP.SERVICES.EQUIPMENT_CATEGORY().GetEquCates({
            ...
            if (typeof data.data !== 'undefined') {
                ...
                if (typeof responseData.ListData !== 'undefined') {
                    this.setState({
                        EquCates: responseData.ListData,
                    });
    ...
    protected handleDeleteEquCate() {
        new APP.SERVICES.EQUIPMENT_CATEGORY().DeleteCate(this.state.SelectedEquCateIdForDeleteAction).then(({data}) => {
            if (data.success) {
                this.handleGetEquCate();
            }
    ...
    protected handleShowDeleteWarning(id: number = 0) {
        const self = this;
        this.setState({
            SelectedEquCateIdForDeleteAction: id,
        }, () => {
            new APP.SERVICES.CORE.ALERT_DIALOG().Show({
                show: true,
                style: {fontSize: 12},
    ...
    public componentDidMount(): void {
        ...
        window.scrollTo(0, 0);
        document.title = "Equipment Category List";
    }
            ...
            <React.Fragment>
                <div style={{marginTop: 15}}>
                    <Row>
                        <Col md={8}>
                            <h4>
                                <Link to={APP.ROUTES.PRIVATE.EQUIPMENT_CATEGORY}>Equipment Category List</Link>
                            </h4>
                        </Col>
                        <Col md={4}>
                            <div style={{textAlign: "right"}}>
                                <Link to={APP.ROUTES.PRIVATE.CREATE_EQUIPMENT_CATEGORY}>
                                    <button className={__css_CreateContractorBtn}>Create Equipment Category</button>
                                </Link>
                            </div>
                        </Col>
                    </Row>
                    <div style={{background: '#ffffff', padding: 5}}>
                        <Table striped bordered hover className={__css_Table} responsive={true} size={"sm"}>
                            ...
                            <tbody>
                            {this.state.EquCates.map((EquCate: any, index: number) => (
                                <tr key={index}>
                                    <td>{index + 1}</td>
                                    <td>{EquCate.name}</td>
                                    <td>
                                        <Link to={APP.ROUTES.PRIVATE.EDIT_EQUIPMENT_CATEGORY + '/' + EquCate.id}><FontAwesome
                                            name={"edit"} className={"grid-action-button grid-action-edit"}/></Link>
                                        <FontAwesome name={"trash"}
                                                     className={"grid-action-button grid-action-delete"}
                                                     onClick={() => this.handleShowDeleteWarning(EquCate.id)}/>
                                    </td>
                                </tr>
                            ))}
                            </tbody>
...
export default withRouter(EquipmentCategoryList);