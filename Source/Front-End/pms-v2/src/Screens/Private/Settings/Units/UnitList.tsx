...
class UnitList extends React.Component<any, any> {
    public state: any;
    ...
    public constructor(props: any) {
        ...
        this.state = {
            ...
            SelectedUnitIdForDeleteAction: 0,
    ...
    protected handleGetUnit() {
        new APP.SERVICES.UNITS().GetUnits({
    ...
    protected handleDeleteUnit() {
        new APP.SERVICES.UNITS().DeleteUnit(this.state.SelectedUnitIdForDeleteAction).then(({data}) => {
            if (data.success) {
                this.handleGetUnit();
            }
    ...
    protected handleShowDeleteWarning(id: number = 0) {
        const self = this;
        this.setState({
            SelectedUnitIdForDeleteAction: id,
        }, () => {
            new APP.SERVICES.CORE.ALERT_DIALOG().Show({
                show: true,
                style: {fontSize: 12},
            ...
            <React.Fragment>
                <div style={{marginTop: 15}}>
                    <Row>
                        <Col md={8}>
                            <h4>
                                <Link to={APP.ROUTES.PRIVATE.UNITS}>Unit List</Link>
                            </h4>
                        </Col>
                        <Col md={4}>
                            <div style={{textAlign: "right"}}>
                                <Link to={APP.ROUTES.PRIVATE.CREATE_UNIT}>
                                    <button className={__css_CreateContractorBtn}>Create Unit</button>
                                </Link>
                            </div>
                        </Col>
                    </Row>
                    <div style={{background: '#ffffff', padding: 5}}>
                        <Table striped bordered hover className={__css_Table} responsive={true} size={"sm"}>
                            ...
                            <tbody>
                            {this.state.Units.map((Unit: any, index: number) => (
                                <tr key={index}>
                                    <td>{index + 1}</td>
                                    <td>{Unit.name}</td>
                                    <td>
                                        <Link to={APP.ROUTES.PRIVATE.EDIT_UNIT + '/' + Unit.id}><FontAwesome
                                            name={"edit"} className={"grid-action-button grid-action-edit"}/></Link>
                                        <FontAwesome name={"trash"}
                                                     className={"grid-action-button grid-action-delete"}
                                                     onClick={() => this.handleShowDeleteWarning(Unit.id)}/>
                                    </td>
                                </tr>
                            ))}
                            </tbody>
